...
@author: Austin Tapp & Krithika Iyer
"""
...
def get_args():
    ...
    parser.add_argument("--compute_brisque", action="store_true")
    parser.add_argument("--compute_clipiqa", action="store_true")
    parser.add_argument("--compute_frd", action="store_true")
...
class Metrics:
    ...
    def frechet_distance(
        ...
        covmean, _ = linalg.sqrtm(
            (sigma_a + eps * np.eye(sigma_a.shape[0])) @ (sigma_b + eps * np.eye(sigma_b.shape[0])),
            disp=False,
        )
        ...
        fid_value = diff.dot(diff) + np.trace(sigma_a) + np.trace(sigma_b) - 2.0 * np.trace(covmean)
    ...
    def brisque_qc(self, slices: Sequence[np.ndarray]) -> float:
        ...
        with torch.no_grad():
            for start in range(0, len(slices), self.args.batch_size):
                ...
                values.append(float(metric(batch).item()))
    ...
    def frd(self, prediction_paths: Sequence[str], reference_paths: Sequence[str]) -> float:
        ...
        with tempfile.TemporaryDirectory(prefix="frd_pred_") as pred_dir, tempfile.TemporaryDirectory(prefix="frd_ref_") as ref_dir:
            ...
            return float(compute_frd([pred_dir, ref_dir]))
...
def extract_case_id(image_path: str) -> str:
    """Extract a stable case identifier from the prediction filename."""
...
def medical_image_metadata_matches(input_path: str, prediction_path: str) -> bool:
    """Return True only when input and prediction medical image metadata match.

    The check is intentionally run before any metric evaluation. It compares
    dimensional/spatial metadata and the SimpleITK metadata dictionary so that
    prediction images cannot alter the acquisition geometry or stored metadata.
    """
    if sitk is None:
    ...
    input_keys = set(input_image.GetMetaDataKeys())
    prediction_keys = set(prediction_image.GetMetaDataKeys())

    if input_keys != prediction_keys:
        return False

    for key in sorted(input_keys):
        if input_image.GetMetaData(key) != prediction_image.GetMetaData(key):
            return False
...
def main():
    """Main function."""
    args = get_args()
    enhanced_paths = collect_image_paths(args.predictions_dir)

    if args.input_dir:
        input_paths = collect_image_paths(args.input_dir, suffixes=MEDICAL_SUFFIXES)
        try:
    ...
    reference_paths = collect_image_paths(args.reference_dir)
    ...
    payload = {
        **get_liveboard_payload(summary),
        "summary": summary,
        "liveboard_metrics": get_liveboard_payload(summary),
        "per_case": case_rows,
    }
